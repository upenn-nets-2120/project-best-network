...
export default function PostComponent({
    ...
    // Function to fetch the initial likes for the post
    const fetchInitialLikes = async () => {
        try {
            const response = await axios.get(`${config.serverRootURL}/getLikes`, {
                params: { post_id },
                withCredentials: true,
            });

            if (response.status === 200) {
                // Set the initial likes state with the value from the server
    ...
    const fetchComments = async () => {
        try {
            ...
            const response = await axios.get(`${config.serverRootURL}/getComments`, {
                params: { post_id },
                withCredentials: true,
            });
