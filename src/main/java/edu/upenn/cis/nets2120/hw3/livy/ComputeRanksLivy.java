...
public class ComputeRanksLivy {
    ...
    public static void main(String[] args)
        ...
        SocialRankJob blJob1 = new SocialRankJob(d_max, i_max, 10, false, debug);
        ...
        List<MyPair<String, Double>> noBacklinksResult = SparkJob.runJob(livy, blJob1);
        ...
        try (PrintStream out = new PrintStream(new FileOutputStream("socialrank-livy-results.txt"))) {
            for (MyPair<String, Double> backItem : backlinksResult) {
                for (MyPair<String, Double> noBackItem : noBacklinksResult) {
                    if (backItem.getLeft().equals(noBackItem.getLeft())) {
                        out.println(backItem.getLeft() + "," + backItem.getRight());
                    }
                }
            }
        } catch (Exception e) {
